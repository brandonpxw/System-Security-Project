...
from flask import Flask, render_template, request, redirect, url_for, session, g, flash, json, Response
from flask_recaptcha import ReCaptcha
from flask_mysqldb import MySQL
from csrf import csrf, CSRFError, CSRFProtect
...
app.config['RECAPTCHA_PUBLIC_KEY'] = '6LeMOvEgAAAAAParXi1z2tXkTU-kiHHMccbrIhwF'
app.config['RECAPTCHA_PRIVATE_KEY'] = '6LeMOvEgAAAAACC0zFp7R1SZGTMgUn6Vnoj5h19y'
...
def is_human(captcha_response):
    ...
    # """
    secret = "6LeMOvEgAAAAACC0zFp7R1SZGTMgUn6Vnoj5h19y"
...
@app.route('/csrfAttack')
def csrfAttack():
    return render_template('/csrfAttack.html')
...
def confirm_email(token):
    ...
    cursor.execute('SELECT * FROM twofactor WHERE email = %s', (email,))
    account2 = cursor.fetchone()
    if account2:
        cur = mysql.connection.cursor()
        cursor.execute('UPDATE twofactor SET ipv4 = %s, hostname = %s, login_time = %s WHERE email = %s', (IPAddress, hostname, datetime.now(), email))

        mysql.connection.commit()
        cur.close()
    else:
        cur = mysql.connection.cursor()
        cur.execute("INSERT IGNORE INTO twofactor (email, logged_in, ipv4, hostname, login_time, reset_time) VALUES (%s, %s, %s, %s, %s, NULL)", (email,'True',IPAddress, hostname, datetime.now()))
...
def handle_csrf_error(e):
    flash("CSRF Token not found! Were you on a sketchy website?", "error")
    return render_template('csrfError.html', reason=e.description), 400


@app.route('/generate_time/<emaildata>')
def generate_time(emaildata):
    def generator():
        while True:
            json_data = json.dumps(
                {'time': datetime.now().strftime('%H:%M:%S'), 'email': emaildata})
...
@app.route('/passwordexpire/<emaildata>')
def passwordexpire(emaildata):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    cursor.execute('SELECT * FROM accounts WHERE email = %s', (emaildata,))
    account1 = cursor.fetchone()
    print(emaildata)
    cursor.execute('UPDATE accounts SET status = %s WHERE email = %s', ('Locked', emaildata,))
    cursor.execute('DROP TABLE forgotAcc')
    mysql.connection.commit()
...
def login(emaildata=None, expirytime=None):
    cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
    login_form = Login(request.form)
    try:
        cursor.execute("SELECT * FROM forgotAcc")
        forget_acc = cursor.fetchone()
        emaildata = forget_acc['email']
        expirytime = forget_acc['expirytime']
    except:
        print("VANNSAR")
    if request.method == 'POST' and login_form.validate():
        ...
        cursor.execute('SELECT * FROM accounts WHERE email = %s', (login_form.email.data,))
        account1 = cursor.fetchone()

        if account1:
            session.permanent = True
            if account1['status'] != 'True':
                session['sql_invalid_password'] = "Your account is locked, please contact us for assistance."
            else:
                encrypted_password = account1['password'].encode()
                ...
                if decrypted_password.decode() == login_form.password.data:
                    newIp = geocoder.ip('')
                    newCountry = newIp.city
                    ...
                    session['email1'] = account1['email']
                    session['sql_login_success'] = account1['title'] + ". " + account1['first_name']
                    cursor.execute('UPDATE accounts SET attempts = %s WHERE email = %s', (0, account1['email'],))
                    # cursor.execute('DROP TABLE forgotAcc')
                    mysql.connection.commit()

                    email = session['email1']
                    token = s.dumps(email, salt="email-confirm")
                    msg = Message('Email Confirmation', sender='appdevresetpw@gmail.com', recipients=[email])
                    link = url_for('confirm_email', token=token, _external=True)
                    msg.body = (f"Click on the link to be redirected. {link}")
                    mail.send(msg)
                    db.close()
                    return render_template("/2fa.html")
                else:
                    cursor.execute('UPDATE accounts SET attempts = %s WHERE email = %s',
                                   (account1['attempts'] + 1, account1['email']))
                    mysql.connection.commit()
                    if account1['attempts'] >= 4:
                        cursor.execute('UPDATE accounts SET total_lockout = total_lockout + 1 where email = %s', (account1['email'],))
                        cursor.execute("UPDATE accounts SET status = 'Locked' WHERE email = %s", (account1['email'],))
                        mysql.connection.commit()
                        session['sql_invalid_password'] = "Your account is locked, please contact us for assistance."
                        return redirect(url_for('login'))
                    session['sql_invalid_password'] = f"Invalid Password, you have {4 - account1['attempts']} attempts left."
                    return redirect(url_for('login'))
                ...
                if login_form.email.data == email:
                    if login_form.password.data == password:
                        if email.split('@')[1] == 'admin.com':
                            ...
                            cursor.execute('SELECT t.email,ipv4,hostname,login_time,reset_time,total_lockout FROM twofactor t inner join accounts a on t.email = a.email;')
                            account3 = cursor.fetchall()
                            return render_template('/accounts.html', account3=account3)

            session['sql_invalid_acc'] = "Invalid account"
            return redirect(url_for('login'))
    ...
    return render_template('/login.html', form=login_form, emaildata=emaildata, expirytime=expirytime)
...
def logout():
    ...
    session.pop('sql_login_success', None)
...
def register():
    ...
    sitekey = "6LeMOvEgAAAAAParXi1z2tXkTU-kiHHMccbrIhwF"
    if request.method == 'POST' and register_form.validate():
        ...
        if register_form.email.data.split('@')[1] == 'admin.com':
            ...
            if recaptcha.verify():
                ...
                password = register_form.password.data.encode()
                ...
                cur = mysql.connection.cursor()
                convertPic = convertToBinary(r'C:\Users\zacha\Downloads\SECPRJ (As of 21.7)\SECPRJ\SECPRJ\static\assets\profiles\profile.png')
                cur.execute("INSERT INTO accounts (first_name, last_name, title, phone, email, password, image, status, make, model, position, attempts, total_lockout) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, NULL, NULL, %s, %s, %s)", (register_form.first_name.data, register_form.last_name.data, register_form.title.data, register_form.phone.data, register_form.email.data, encrypted_password, convertPic, 'True', 'None', 0, 0))

                mysql.connection.commit()
                cur.close()
            else:
                # Log invalid attempts
                session['not_human'] = "You are not a human"
                return redirect(url_for('register'))
    ...
    if request.method == 'POST' and carregister_form.validate():
        ...
        if carregister_form.email.data in emails:
            ...
            if recaptcha.verify():
                ...
                password = carregister_form.password.data.encode()
                key = Fernet.generate_key()
                with open("symmetric.key", 'wb') as fo:
                    fo.write(key)
                f = Fernet(key)
                ...
                convertPic = convertToBinary(
                    r'C:\Users\isaac\PycharmProjects\SECPRJ\static\assets\profiles\profile.png')
                cur.execute("INSERT INTO accounts (first_name, last_name, title, phone, email, password, image, status, make, model, position, attempts) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)",(carregister_form.first_name.data, carregister_form.last_name.data, carregister_form.title.data,carregister_form.phone.data, carregister_form.email.data, encrypted_password, convertPic, 'True', carregister_form.make.data, carregister_form.model.data, 'None', 0))
...
def tables():
    ...
    cur = mysql.connection.cursor()
    accounts = cur.execute("SELECT image, title, first_name, last_name, email, phone, status FROM accounts WHERE make IS NULL")
    if accounts > 0:
    ...
    for key in accounts_dict:
        account = accounts_dict.get(key)
        if account.get_email().split('@')[1] != 'admin.com' and account.get_email().split('@')[1] != 'biazy.com':
            accounts_list.append(account)
...
def password():
    ...
    if request.method == 'POST' and forget_password_form.validate():
        ...
        cursor = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cursor.execute('SELECT * FROM accounts WHERE email = %s', (forget_password_form.email.data,))
        account1 = cursor.fetchone()

        temporary_password = str(uuid4())[:8]

        if account1:
            temporary_password = temporary_password.encode()
            key = Fernet.generate_key()
            with open("symmetric.key", 'wb') as fo:
                fo.write(key)
            f = Fernet(key)
            encrypted_password = f.encrypt(temporary_password)
            cursor.execute('UPDATE accounts SET password = %s WHERE email = %s', (encrypted_password, account1['email'],))
            mysql.connection.commit()
        ...
        for account in accounts:
            if forget_password_form.email.data == account.get_email():
                ...
                cur = mysql.connection.cursor()
                cursor.execute('UPDATE twofactor SET reset_time = %s WHERE email = %s', (reset_time, forget_password_form.email.data))
                ...
                cursor.execute("DROP TABLE IF EXISTS forgotAcc")
                cursor.execute("""CREATE TABLE forgotAcc (
                                  email varchar(200),
                                  expirytime varchar(100)
                                  )""")
                cursor.execute("INSERT INTO forgotAcc Values(%s, %s)", (forget_password_form.email.data, expirytime,))
                mysql.connection.commit()
                return redirect(url_for('login', emaildata=forget_password_form.email.data))
        session['email_not_found'] = forget_password_form.email.data
        return redirect(url_for('password'))

    else:
        print('error')
    return render_template('/password.html', form=forget_password_form, expirytime=expirytime)
